...
import {
  ArrowLeft, BookOpen, ChevronRight, Navigation, Users, HelpCircle, LayoutDashboard, Menu, Bell, Home, BookCheck, GalleryVerticalEnd, Layers, MessageSquareMore, FolderClock, UserCog, UserCheck as UserCheckIcon, User as UserIcon, History, FilePlus, FileSymlink, UploadCloud, FileText as FileTextIcon, Link as LinkIcon, Eye, Trash2, Database, Upload, PieChart, BarChart3, AreaChart, Gauge, CopyPlus, SearchX, FileUp // Adicionado FileUp e outros icones
} from "lucide-react";
import { HiOutlineLogout } from "react-icons/hi"; // Manter se usado em algum content component
import Image from 'next/image'; // Manter se usado em algum content component
import { Button } from "@/components/ui/button"; //
import { cn } from "@/lib/utils"; //
import { SearchBar } from "@/components/ui/search-bar"; //
import { useDebounce } from "@/hooks/useDebounce"; //

// --- Componentes de Conteúdo dos Tópicos (Colar aqui os componentes definidos anteriormente) ---
    ...
    <div className="space-y-8">
        ...
        <section className="prose max-w-none text-gray-700 space-y-4">
            ...
            <ol className="list-decimal pl-5 space-y-3">
                <li><strong>Abra o Modal:</strong> Clique no botão &ldquo;Adicionar Contexto&rdquo; (ícone <FilePlus className="inline w-4 h-4 mx-1"/>) na página da sua gerência ou na seção de validação/page.tsx].</li>
                <li><strong>Selecione a Aba &ldquo;Contexto&rdquo;:</strong> Certifique-se de que a aba &ldquo;Contexto&rdquo; (ícone <FilePlus className="inline w-4 h-4 mx-1"/>) esteja selecionada no topo do modal.<br/><span className="text-xs italic text-gray-500">[IMAGEM: Modal Add Contexto com aba Contexto destacada]</span></li>
                ...
                <li><strong>Anexar Fonte:</strong> Escolha <strong>uma</strong> das opções:
                    <ul className="list-disc pl-5 mt-2 space-y-2">
                        <li><strong>Arquivo (<UploadCloud className="inline w-4 h-4 mx-1"/>):</strong> Arraste e solte o arquivo desejado (PDF, DOCX, XLSX, etc.) na área indicada, ou clique nela para selecionar o arquivo do seu computador. O nome e tamanho do arquivo aparecerão.<br/><span className="text-xs italic text-gray-500">[IMAGEM: Área de Arrastar/Soltar ou seleção de arquivo]</span></li>
                        <li><strong>Link (<LinkIcon className="inline w-4 h-4 mx-1"/>):</strong> Clique no botão com ícone de link (🔗). Uma caixa de diálogo pedirá a URL completa. Cole o endereço web (ex: <span className="italic">https://site.com/documento.pdf</span>). O link inserido será exibido na área de anexo.</li>
                    </ul>
   ...
   <div className="space-y-8">
        ...
        <section className="prose max-w-none text-gray-700 space-y-4">
            ...
            <ol className="list-decimal pl-5 space-y-3">
                ...
                <li><strong>Selecione a Aba &ldquo;Dashboard&rdquo;:</strong> Escolha a aba &ldquo;Dashboard&rdquo; (ícone <LayoutDashboard className="inline w-4 h-4 mx-1"/>) no topo do modal.</li>
                ...
                <li><strong>Escolha o Tipo de Gráfico:</strong> Selecione entre Pizza (<PieChart className="inline w-4 h-4 mx-1"/>), Barras (<BarChart3 className="inline w-4 h-4 mx-1"/>) ou Área/Linha (<AreaChart className="inline w-4 h-4 mx-1"/>). A estrutura da tabela de dados se ajustará ligeiramente.<br/><span className="text-xs italic text-gray-500">[IMAGEM: Seletor de tipo de gráfico destacando as opções]</span></li>
                <li><strong>Fonte dos Dados:</strong> Escolha como fornecer os dados:
                    <ul className="list-disc pl-5 mt-2 space-y-2">
                        ...
                        <li><strong>Upload de Arquivo (<Upload className="inline w-4 h-4 mx-1"/>):</strong> Baixe o arquivo modelo CSV clicando em <strong>&ldquo;Baixar template de exemplo&rdquo;</strong>. Preencha o CSV com seus dados (mantendo os cabeçalhos) e faça o upload clicando na área indicada ou arrastando o arquivo.<br/><span className="text-xs italic text-gray-500">[IMAGEM: Área de upload de arquivo com botão de template]</span></li>
                    </ul>
                </li>
                <li><strong>Tema de Cores (Opcional):</strong> Escolha uma paleta de cores predefinida para o seu gráfico.</li>
                <li><strong>Gerar Pré-visualização:</strong> Clique no botão <strong>&ldquo;Gerar Gráfico&rdquo; / &ldquo;Atualizar Gráfico&rdquo;</strong> para ver como ele ficará. Você pode visualizar em tela cheia (<Expand className="inline w-4 h-4 mx-1"/>).<br/><span className="text-xs italic text-gray-500">[IMAGEM: Pré-visualização do gráfico com botão de atualizar]</span></li>
   ...
   <div className="space-y-8">
        <header className="mb-6">
         ...
         <section className="prose max-w-none text-gray-700 space-y-4">
            ...
            <ol className="list-decimal pl-5 space-y-3">
                <li><strong>Abra o Modal:</strong> Clique em &ldquo;Adicionar Contexto&rdquo; (<FilePlus className="inline w-4 h-4 mx-1"/>).</li>
                <li><strong>Selecione a Aba &ldquo;Indicador&rdquo;:</strong> Escolha a aba &ldquo;Indicador&rdquo; (ícone <Gauge className="inline w-4 h-4 mx-1"/>) no topo do modal.<br/><span className="text-xs italic text-gray-500">[IMAGEM: Modal Add Contexto com aba Indicador destacada]</span></li>
                ...
                <li><strong>Unidade:</strong> Selecione a unidade de medida na lista (%, R$, Pessoas, Dias, Nenhum, etc.).<br/><span className="text-xs italic text-gray-500">[IMAGEM: Dropdown de Unidades]</span></li>
                ...
                <li><strong>Ícone e Cor:</strong> Escolha um ícone (<Heart className="inline w-4 h-4 mx-0.5"/>, <Users className="inline w-4 h-4 mx-0.5"/>, etc.) e uma cor que representem o indicador visualmente. A cor será usada na borda do card.<br/><span className="text-xs italic text-gray-500">[IMAGEM: Seletores de Ícone e Cor]</span></li>
                <li><strong>Pré-visualização:</strong> Um card de exemplo será atualizado automaticamente à medida que você preenche os campos, mostrando como o indicador final aparecerá.<br/><span className="text-xs italic text-gray-500">[IMAGEM: Pré-visualização do card do indicador]</span></li>
        ...
        <header className="mb-6">
         ...
         <section className="prose max-w-none text-gray-700 space-y-4">
            ...
            <ol className="list-decimal pl-5 space-y-3">
                ...
                <li><strong>Vá para a Aba &ldquo;Versões&rdquo;:</strong> Clique na aba &ldquo;Versões&rdquo; (ícone <History className="inline w-4 h-4 mx-1"/>).<br/><span className="text-xs italic text-gray-500">[IMAGEM: Modal de Visualização com aba Versões destacada]</span></li>
                <li><strong>Clique em &ldquo;Criar Nova Versão&rdquo;:</strong> Se você tiver permissão, verá um botão &ldquo;+ Criar Nova Versão&rdquo;. Clique nele.</li>
                <li><strong>Modal de Nova Versão:</strong> O modal de &ldquo;Adicionar Contexto&rdquo; abrirá, mas pré-preenchido com as informações da versão anterior (Título, tipo, etc.) e marcado como &ldquo;NOVA VERSÃO&rdquo;.<br/><span className="text-xs italic text-gray-500">[IMAGEM: Modal Add Contexto no modo Nova Versão]</span></li>
                <li><strong>Atualize a Fonte:</strong> Anexe o <strong>novo arquivo</strong>, insira o <strong>novo link</strong>, atualize os <strong>dados manuais</strong> do gráfico ou insira o <strong>novo valor</strong> do indicador. <strong>Este passo é obrigatório.</strong>.</li>
                <li><strong>Descreva a Alteração:</strong> Selecione o <strong>Motivo da Alteração</strong> (Ex: &ldquo;Correção&rdquo;, &ldquo;Atualização Mensal&rdquo;) e escreva uma <strong>Descrição das Alterações</strong> detalhando o que mudou nesta versão (Ex: <span className="italic">&ldquo;Atualizados dados para Outubro/2025&rdquo;</span>, <span className="italic">&ldquo;Corrigido valor da Linha 5&rdquo;</span>). <strong>A descrição é obrigatória</strong>.</li>
...
const allHelpData: HelpArea[] = [
  ...
  },
  {
    ...
    icon: FilePlus,
    topicos: [
      // <<< Títulos Atualizados para remover aspas literais >>>
...
// <<< ATUALIZADO para 'gerenciando-conteudo' >>>
...
export default function GerenciandoConteudoPage() {
  ...
  // Lógica de Busca Global (sem alterações)
  const globalFilteredResults = useMemo(() => {
    ...
    allHelpData.forEach(area => {
      area.topicos.forEach(topico => {
        const matchesTitle = topico.titulo.toLowerCase().includes(lowerCaseSearch);
        ...
        if (matchesTitle || matchesDesc || matchesKeywords) {
          results.push({ ...topico, titulo: topico.titulo.replace(/"/g, ''), areaNome: area.nome });
        }
      });
    });
    return results;
  ...
  // Efeito para fechar dropdown (sem alterações)
  useEffect(() => {
  ...
  // Seleção de tópico (sem alterações na lógica principal)
  const handleTopicSelect = (href: string) => {
      ...
      if (isCurrentArea) {
        ...
        // Navegação via Link do dropdown cuidará disso
        setSearchValue("");
        setIsSearchFocused(false);
      }
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="border-b border-gray-200 bg-blue-50 sticky top-16 z-10">
        <div className="max-w-6xl mx-auto px-6 py-4">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            {/* Container da Barra de Busca com Ref */}
            <div ref={searchContainerRef} id="ajuda-search-container" className="w-full md:w-auto relative">
               <SearchBar
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 md:py-12 flex flex-col md:flex-row gap-8 lg:gap-12 relative">
          ...
          <div className="sticky top-40">
            ...
            <nav className="space-y-1">
              {currentAreaTopics.map((topico) => {
                ...
                // <<< CORREÇÃO: Usar topicIconMap aqui >>>
                ...
                return (
                  ...
                  >
                    ...
                    {/* <<< CORREÇÃO: Título sem aspas literais >>> */}
                    <span className="truncate">{topico.titulo.replace(/"/g, '')}</span>
                  </button>
                );
              })}
              {/* Mensagens de busca (sem alterações) */}
        ...
        {/* <<< Adicionado overflow-y-auto e scroll-pt aqui >>> */}
        <main id="main-content-area" className="flex-1 min-w-0 overflow-y-auto" style={{ scrollPaddingTop: '10rem', maxHeight: 'calc(100vh - 10rem)' }}> {/* Ajuste maxHeight conforme necessário */}
