                    ...
                    {conjuntoDeDados.colunas.slice(1).map((coluna, index) => (
                        <div key={index} className="flex items-center gap-2 bg-white p-1.5 rounded-2xl border border-gray-200">
                            ...
                            {conjuntoDeDados.colunas.length > 2 && (
                                <button onClick={() => aoRemoverColuna(index + 1)} className="p-1.5 rounded-md text-gray-400 hover:bg-red-50 hover:text-red-600 transition">
            ...
            <div ref={scrollContainerRef} className="overflow-auto max-h-64 rounded-2xl border border-gray-200 bg-white">
                <table className="w-full text-sm">
                    <thead className="bg-gray-100 sticky top-0 z-10">
                        <tr>
                            <th className="p-3 text-left w-1/3 min-w-[200px]"><EntradaTabelaDeDados eCabecalho valor={conjuntoDeDados.colunas[0]} aoMudar={(valor) => aoAtualizarNomeColuna(0, valor)} placeholder="Categoria" className="text-left" /></th>
                            {conjuntoDeDados.colunas.slice(1).map((coluna, index) => ( <th key={index} className="p-3 text-center min-w-[150px] font-medium text-gray-600">{coluna || `Série ${index + 1}`}</th> ))}
                    ...
                    <tbody>
                        {conjuntoDeDados.linhas.map((linha, indiceLinha) => (
                            <tr key={indiceLinha} className="border-t border-gray-200 group">
                                <td className="p-2 w-1/3 min-w-[200px]"><EntradaTabelaDeDados valor={linha[0]} aoMudar={(valor) => aoAtualizarCelula(indiceLinha, 0, valor)} placeholder="Ex: Janeiro" /></td>
                                {linha.slice(1).map((celula, indiceCelula) => (
                                    <td key={indiceCelula} className="p-2 min-w-[150px]"><EntradaTabelaDeDados valor={celula} aoMudar={(valor) => aoAtualizarCelula(indiceLinha, indiceCelula + 1, valor)} placeholder="0" tipo="number" /></td>
                                ))}
                                <td className="w-12 text-center">
                                    {conjuntoDeDados.linhas.length > 1 && ( <button onClick={() => aoRemoverLinha(indiceLinha)} className="text-gray-400 hover:text-red-600 p-1 rounded-full opacity-0 group-hover:opacity-100 transition"><Trash2 className="w-4 h-4" /></button> )}
                                </td>
                            </tr>
                        ))}
                    </tbody>
