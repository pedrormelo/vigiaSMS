...
const EmptyFilterState = ({ filter }: { filter: ActiveFilter }) => {
  const config = {
    all: { icon: <Inbox className="h-10 md:h-12 w-10 md:w-12 mb-3 md:mb-4 text-gray-300" />, text: "Nenhuma notificação encontrada." },
    unread: { icon: <CheckCircle className="h-10 md:h-12 w-10 md:w-12 mb-3 md:mb-4 text-green-300" />, text: "Nenhuma notificação não lida." },
    system: { icon: <Settings className="h-10 md:h-12 w-10 md:w-12 mb-3 md:mb-4 text-blue-300" />, text: "Nenhuma notificação do sistema." }
  };
...
export default function NotificationList({
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white shadow-sm">
      ...
      <div className="flex-shrink-0 p-2 md:p-4 border-b border-gray-200 bg-white">
        <div className="inline-flex items-center gap-1.5 md:gap-2 mb-1 text-blue-800">
          <h3 className="text-sm md:text-lg font-semibold text-blue-800">Caixa de Entrada</h3>
          <button
          ...
          >
            <Settings2 className="h-3.5 md:h-5 w-3.5 md:w-5" />
          </button>
        </div>
      ...
      <div className="flex-1 w-full h-full overflow-y-auto scrollbar-custom">
        {notifications.length > 0 ? (
          <div className="flex flex-col gap-1 p-1.5 md:p-3">
