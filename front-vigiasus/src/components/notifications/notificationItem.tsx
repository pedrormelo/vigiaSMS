...
export default function NotificationItem({
  ...
  return (
    ...
    >
      ...
      {!isRead && (
        <span 
          className="absolute top-3 left-3 block h-2.5 w-2.5 rounded-full bg-blue-500 ring-2 ring-white" 
          aria-label="Não lida" 
        />
      )}
      ...
      {/* Container do Ícone */}
      <div className={cn(
        "w-10 h-10 flex-shrink-0 flex items-center justify-center rounded-full mt-0.5",
         isActive ? "bg-white" : "bg-gray-100", // Fundo do ícone
      ...
      <div className="flex-1 min-w-0">
        <h3
          className={cn(
            "font-semibold text-sm leading-snug text-gray-800 line-clamp-2",
        ...
        <p
          className={cn(
            ...
            isActive ? "text-gray-700" : statusColor // Usa a cor de status na descrição
          )}
           title={description}
        >
          {description}
